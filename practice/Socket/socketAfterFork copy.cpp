		...
		if (childFd == 0) {
			...
			while (1) 
			{
				...
				for (int i = 0; i < retEvents; ++i) {
					std::cout << "event size is " << retEvents << std::endl;
					if (change_list[i].filter == EVFILT_READ) {
						...
						if (change_list[i].ident == sockfd) {
							...
							int n = read(change_list[i].ident, tmpBuf, sizeof(tmpBuf));
							if (n <= 0) {
								struct kevent event;
							...
							struct kevent event;
							EV_SET(&event, change_list[i].ident, EVFILT_WRITE, EV_CLEAR , 0, 0, NULL);
							event_list.push_back(event);
